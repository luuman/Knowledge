### 平时怎么学习(淘系）
### PWA原理，讲下核心点（淘系）
[PWA原理](https://blog.csdn.net/weixin_33692284/article/details/88027509)

### 简述TCP连接的过程（淘系）
[简述TCP连接的过程](https://blog.csdn.net/qq_42196196/article/details/81042376)
1. 建立三次握手、释放四次握手，发送请求，回复请求，建立连接，关闭请求，发送回复，关闭数据传输、发送确认。
...
1. bool类型、object类型的参数，事件冒泡；事件捕获、自动销毁
...
### 加载模块时，为什么每个模块都有__dirname, filename属性呢，这两个属性是从哪里来的
### 请介绍一下require的模块加载机制
### 介绍一下node里的模块是什么
### 如何实现内网穿透，端口转发了解一下？
...
### v8有了解过吗？讲讲了解过v8的那几个模块和部分，比如解释一下v8的hidden class
...
### 如何用css实现一个三角形
### 如何实现一个自适应的正方形
### 如何实现一个左右固定，中间自适应的三栏布局
### 10w 条记录的数组，一次性渲染到页面上，如何处理可以不冻结UI？
### 介绍下浏览器事件委托
### 说下 [1, 2, 3].map(parseInt) 结果
### import 和 link 区别
### 说说你对 BFC 的理解
### 清除浮动的方法
### 说说选择器的权重计算方式
